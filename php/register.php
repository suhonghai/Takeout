<?php 
	...
	//验证是否已注册
	$sql = "SELECT uid, email, firstname, lastname, score, level, createtime FROM `mall_users` WHERE phone='$phone'";
	// 执行查询SQL语句，返回查询结果集（类似表格）
	$result = mysql_query($sql);

	if($row = mysql_fetch_array($result, MYSQL_ASSOC)){
		echo '{"res_code":1, "res_error":"", "res_body": '. json_encode($row) .'}';
	}else{
		// 创建插入语句
		$sql = "INSERT INTO mall_users (phone,firstname,password) VALUES('$phone','$firstname','$password')";
